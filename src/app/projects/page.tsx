import { getRepos, getContributors, categorizeRepos, type GitHubRepo, type GitHubContributor } from "@/lib/github";
import Link from "next/link";
import { ArrowLeft, Star, GitFork, AlertCircle, ExternalLink, Calendar } from "lucide-react";
...
function RepoCard({ repo, contributors }: { repo: GitHubRepo; contributors: GitHubContributor[] }) {
  return (
    <a
      href={repo.html_url}
      target="_blank"
      ...
      {contributors.length > 0 && (
        <div className="mt-4 pt-3 border-t border-white/[0.05] flex items-center gap-2">
          ...
          <span className="text-xs text-white/30">
            {contributors.length > 5 ? `${contributors.length} contributors` : `${contributors.length} contributor${contributors.length !== 1 ? "s" : ""}`}
          </span>
        </div>
      )}
    </a>
